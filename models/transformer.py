...
class GPT(nn.Module):
    ...
    def encode_prototypes(self, parameters):
        ...
        for parameter, in_proj in zip(parameters, self.input_prototype_projections):
            proj = in_proj(parameter)

            representations.append(proj)
...
class Gpt(nn.Module):
    ...
    def forward(self, x, x_ori, t):
        ...
        inp = [torch.unsqueeze(x_norm, dim=1), x_ori.view(x_ori.shape[0], 1, x_ori.shape[1]),
               torch.unsqueeze(t_embedding, dim=1)]
